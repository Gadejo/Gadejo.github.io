...
async function loginUser(db, email, password, request) {
  ...
  try {
    ...
    const profile3 = await db.prepare("SELECT * FROM user_profiles WHERE user_id = ?").bind(user.id).first();
    return new Response(JSON.stringify({
      success: true,
      user: {
        id: user.id,
        ...
        level: profile3?.level || 1,
        totalXp: profile3?.total_xp || 0
      },
      token,
...
var onRequestPost2 = /* @__PURE__ */ __name(async (context2) => {
  ...
  try {
    ...
    console.error("Data operation error:", error3);
    return new Response(JSON.stringify({ error: "Internal server error" }), {
      status: 500,
      headers: { "Content-Type": "application/json" }
    });
...
async function loadAppData(db, userId) {
  try {
    ...
    subjectConfigs.results?.forEach((config2) => {
      const subjectData = subjects.results?.find((s) => s.id === config2.id);
      subjectsData[config2.id] = {
        config: {
          id: config2.id,
          name: config2.name,
          emoji: config2.emoji,
          color: config2.color,
          achievements: JSON.parse(config2.achievements),
          questTypes: JSON.parse(config2.quest_types),
          pipAmount: config2.pip_amount,
          targetHours: config2.target_hours,
          resources: JSON.parse(config2.resources),
          customFields: JSON.parse(config2.custom_fields || "{}")
        },
    ...
    if (Object.keys(subjectsData).length === 0) {
      ...
      newSubjectConfigs.results?.forEach((config2) => {
        const subjectData = newSubjects.results?.find((s) => s.id === config2.id);
        subjectsData[config2.id] = {
          config: {
            id: config2.id,
            name: config2.name,
    ...
    console.error("Load app data error:", error3);
  ...
  try {
    ...
    if (appData.subjects && typeof appData.subjects === "object") {
      for (const [subjectId, subject] of Object.entries(appData.subjects)) {
        ...
        let config2, subjectData;
        if (subject && typeof subject === "object") {
          if (subject.config) {
            config2 = subject.config;
            subjectData = { ...subject };
        ...
        const safeConfig = {
          id: config2.id || subjectId,
          name: config2.name || "Unknown Subject",
          emoji: config2.emoji || "\u{1F4DA}",
          color: config2.color || "#3B82F6",
          pipAmount: config2.pipAmount || 5,
          targetHours: config2.targetHours || 8,
          achievements: config2.achievements || [],
          questTypes: config2.questTypes || ["study"],
          resources: config2.resources || [],
          customFields: config2.customFields || {}
        };
        operations.push(
          db.prepare(`
    ...
    console.error("Save app data error:", error3);
...
async function addSession(db, userId, session) {
  try {
    ...
    console.error("Add session error:", error3);
...
async function updateSubject(db, userId, subjectId, updates) {
  try {
    ...
    console.error("Update subject error:", error3);
...
async function addGoal(db, userId, goal) {
  try {
    ...
    console.error("Add goal error:", error3);
...
async function updateGoal(db, userId, goalId, updates) {
  try {
    ...
    console.error("Update goal error:", error3);
...
async function deleteGoal(db, userId, goalId) {
  try {
    ...
    console.error("Delete goal error:", error3);
...
async function setPipCount(db, userId, pipData) {
  try {
    ...
    console.error("Set pip count error:", error3);
    ...
    if (appData.pips && typeof appData.pips === "object") {
      for (const [date, subjectPips] of Object.entries(appData.pips)) {
        if (subjectPips && typeof subjectPips === "object") {
          for (const [subjectId, count3] of Object.entries(subjectPips)) {
            if (typeof count3 === "number" && count3 > 0) {
              batchOps.push(
                db.prepare(`
                  INSERT INTO pips (user_id, date, subject_id, count)
                  VALUES (?, ?, ?, ?)
                `).bind(userId, date, subjectId, count3)
              );
            }
          }
        }
...
async function testMigration(db, userId, localStorageData) {
  try {
    ...
    console.error("Test migration error:", error3);
    return new Response(JSON.stringify({
...
async function stepByStepMigration(db, userId, localStorageData) {
  ...
  try {
    ...
    console.error("Step-by-step migration error:", error3);
...
async function runDiagnostics(db, token, userId) {
  ...
  try {
    ...
    console.error("Diagnostics error:", error3);
...
async function ensureUserExists(db, userId) {
  try {
    ...
    console.error("Ensure user exists error:", error3);
    return new Response(JSON.stringify({
...
var onRequestPost3 = /* @__PURE__ */ __name(async (context2) => {
  ...
  try {
    ...
    switch (action) {
      case "store":
        ...
        try {
          ...
          console.error("KV store error:", error3);
          return new Response(JSON.stringify({
            success: false,
      ...
      case "get":
        ...
        try {
          ...
          console.error("KV get error:", error3);
          return new Response(JSON.stringify({
            success: false,
      ...
      case "delete":
        ...
        try {
          ...
          console.error("KV delete error:", error3);
          return new Response(JSON.stringify({
            success: false,
      ...
      case "list":
        try {
          ...
          console.error("KV list error:", error3);
          return new Response(JSON.stringify({
            success: false,
    ...
    console.error("KV Session API error:", error3);
    return new Response(JSON.stringify({
      success: false,
...
var onRequestPost5 = /* @__PURE__ */ __name(async (context2) => {
  ...
  try {
    ...
    console.error("Template operation error:", error3);
    return new Response(JSON.stringify({ error: "Internal server error" }), {
      status: 500,
      headers: { "Content-Type": "application/json" }
    });
...
async function loadUserTemplates(db, userId) {
  try {
    ...
    console.error("Load user templates error:", error3);
...
async function saveUserTemplate(db, userId, template) {
  try {
    ...
    console.error("Save user template error:", error3);
...
async function deleteUserTemplate(db, userId, templateId) {
  try {
    ...
    console.error("Delete user template error:", error3);
...
__name(verifyTokenAndGetUserId2, "verifyTokenAndGetUserId");
...
var onRequest = /* @__PURE__ */ __name(async (context2) => {
  ...
  if (url.pathname.startsWith("/api/")) {
    ...
    const rateLimitWindow = parseInt(env2.RATE_LIMIT_WINDOW);
    const rateLimitRequests = parseInt(env2.RATE_LIMIT_REQUESTS);
    try {
      const currentCount2 = await env2.RATE_LIMIT.get(rateLimitKey);
      ...
      await env2.RATE_LIMIT.put(rateLimitKey, (count4 + 1).toString(), {
        expirationTtl: rateLimitWindow
      });
    } catch (error3) {
      console.error("Rate limiting error:", error3);
    }
    const response2 = await context2.next();
    ...
    const currentCount = await env2.RATE_LIMIT.get(rateLimitKey);
    const count3 = currentCount ? parseInt(currentCount) : 0;
    newResponse2.headers.set("X-RateLimit-Limit", rateLimitRequests.toString());
    newResponse2.headers.set("X-RateLimit-Remaining", Math.max(0, rateLimitRequests - count3).toString());
    newResponse2.headers.set("X-RateLimit-Reset", (Date.now() + rateLimitWindow * 1e3).toString());
    return newResponse2;
  }
  if (env2.NODE_ENV === "production" && (url.pathname.endsWith(".js") || url.pathname.endsWith(".css") || url.pathname.endsWith(".png") || url.pathname.endsWith(".jpg") || url.pathname.endsWith(".svg") || url.pathname.endsWith(".ico"))) {
    const response2 = await context2.next();
    const newResponse2 = new Response(response2.body, response2);
...
var onRequestOptions = /* @__PURE__ */ __name(async ({ env: env2 }) => {
...
// ../.wrangler/tmp/pages-YCQnts/functionsRoutes-0.6339076628685194.mjs
var routes = [
  {
    routePath: "/api/auth",
    mountPath: "/api",
...
//# sourceMappingURL=functionsWorker-0.6319389083192735.mjs.map
